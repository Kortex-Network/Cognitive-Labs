...
const schema = yup.object().shape({
  cognitiveLabIdentifier: yup
    .string()
    .required("Cognitive Lab Identifier is required")
...
const STEPS = ["Connect Wallet", "Configure Cognitive Lab", "Submit"];
...
const CreateCognitiveLab = () => {
  ...
  const currentIdentifier = watch("cognitiveLabIdentifier");
  ...
  const handleCreateCognitiveLab = async (data) => {
    setLoading(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      // Construct the full cognitive lab URL using the validated identifier
      const fullCognitiveLabId = `LABS:stellar:${wallet.publicKey}:${data.cognitiveLabIdentifier}`;

      const response = await stellarAPI.contracts.registerCognitiveLab({
        LABS: fullCognitiveLabId,
      ...
      toast.success("Cognitive Lab created successfully!");
      reset();
    } catch (err) {
  ...
  return (
    <Box component="main" aria-label="Create Cognitive Lab page" sx={{ px: { xs: 2, sm: 3 }, py: { xs: 2, sm: 3 } }}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        Create a new Cognitive Lab on the Stellar blockchain to manage your digital
        identity
      </Typography>
      ...
      {loading && (
        <Box sx={{ mb: 2 }}>
          <LinearProgress aria-label="Submitting Cognitive Lab creation request" />
      ...
      <Grid container spacing={3} role="region" aria-label="Create Cognitive Lab form">
        {/* Wallet Connection */}
              ...
              {!isConnected ? (
                <Box>
                  <Typography
                    ...
                    sx={{ mb: 2 }}
                  >
                    Connect your wallet to create a Cognitive Lab
                  </Typography>
                  <Button
                    variant="contained"
        ...
        <Grid item xs={12} md={6}>
          <Card>
            <CardContent>
              ...
              <form
                onSubmit={handleSubmit(handleCreateLABS)}
                aria-label="Create LABS form"
                noValidate
              >
                <Controller
                  name="LABSIdentifier"
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      fullWidth
                      margin="normal"
                      error={!!errors.LABSIdentifier}
                      helperText={
                        errors.LABSIdentifier?.message ||
                        "Alphanumeric and hyphens only (3–32 chars)"
                      }
                      InputProps={{
                        endAdornment: (
                          <FieldStatusAdornment
                            isTouched={!!touchedFields.LABSIdentifier}
                            hasError={!!errors.LABSIdentifier}
                          />
                        ),
                      }}
                    />
                ...
                >
                  ...
                  >
                    LABS:stellar:{wallet?.publicKey || "G..."}
                  ...
                  render={({ field }) => (
                    <TextField
                      ...
                      helperText={
                        ...
                        "Optional: public URL for your LABS service"
                      }
                      InputProps={{
                        endAdornment: (
                          <FieldStatusAdornment
                ...
                <Button
                  ...
                  fullWidth
                  sx={{ mt: 2 }}
                  aria-label={loading ? "Creating LABS, please wait" : "Create LABS"}
                >
                  {loading ? (
                  ...
                  <Grid item xs={12} md={6}>
                    <Paper sx={{ p: 2, bgcolor: "background.default" }}>
                      ...
                      <Box display="flex" alignItems="center">
                        ...
                        <IconButton
                          size="small"
                          onClick={() => copyToClipboard(result.data.LABS)}
                        >
                          <ContentCopy fontSize="small" />
                        </IconButton>
                      </Box>
