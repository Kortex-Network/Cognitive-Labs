...
import "../contracts/IntegratedLABSRegistry.sol";
import "../contracts/access/EnhancedAccessControl.sol";
import "../contracts/proxy/EnhancedProxy.sol";
import "../contracts/optimized/GasOptimizedLABSRegistry.sol";

/**
 * @title IntegratedLABSRegistryTest
 * @dev Comprehensive test suite for the integrated LABS registry solution
 * 
 ...
 */
contract IntegratedLABSRegistryTest is Test {
    
    // ===== TEST CONTRACTS =====
    
    IntegratedLABSRegistry public integratedRegistry;
    EnhancedAccessControl public accessControl;
    EnhancedProxy public proxy;
    GasOptimizedLABSRegistry public gasOptimizedRegistry;
    ...
    string public constant TEST_LABS = "LABS:ethereum:0x1234567890123456789012345678901234567890";
    string public constant TEST_PUBLIC_KEY = "0x04abc123def456ghi789jkl012mno345pqr678stu901vwx234yz567abc890def123";
    string public constant TEST_SERVICE_ENDPOINT = "https://LABS.example.com/endpoint";
    string public constant TEST_ISSUER = "LABS:ethereum:0x0987654321098765432109876543210987654321";
    string public constant TEST_SUBJECT = "LABS:ethereum:0x1111111111111111111111111111111111111111";
    ...
    function setUp() public {
        ...
        vm.prank(owner);
        gasOptimizedRegistry = new GasOptimizedLABSRegistry(address(accessControl));
        
        // Deploy integrated registry
        vm.prank(owner);
        integratedRegistry = new IntegratedLABSRegistry();
    ...
    function test_RBAC_AdminRoleHasAllPermissions() public {
        ...
        assertTrue(accessControl.checkPermission(owner, ResourceType.LABS, OperationType.CREATE));
        assertTrue(accessControl.checkPermission(owner, ResourceType.LABS, OperationType.READ));
        assertTrue(accessControl.checkPermission(owner, ResourceType.LABS, OperationType.UPDATE));
        assertTrue(accessControl.checkPermission(owner, ResourceType.LABS, OperationType.DELETE));
    ...
    function test_RBAC_GovernorPermissions() public {
        ...
        assertTrue(accessControl.checkPermission(governor, ResourceType.LABS, OperationType.VALIDATE));
    ...
    function test_RBAC_IssuerPermissions() public {
        ...
        assertTrue(accessControl.checkPermission(issuer, ResourceType.LABS, OperationType.READ));
    ...
    function test_RBAC_UserPermissions() public {
        // User should have basic LABS permissions
        assertTrue(accessControl.checkPermission(user, ResourceType.LABS, OperationType.CREATE));
        assertTrue(accessControl.checkPermission(user, ResourceType.LABS, OperationType.READ));
        assertTrue(accessControl.checkPermission(user, ResourceType.LABS, OperationType.UPDATE));
        assertTrue(accessControl.checkPermission(user, ResourceType.LABS, OperationType.DELETE));
    ...
    function test_RBAC_AuditorPermissions() public {
        ...
        assertTrue(accessControl.checkPermission(auditor, ResourceType.LABS, OperationType.READ));
        ...
        assertFalse(accessControl.checkPermission(auditor, ResourceType.LABS, OperationType.CREATE));
    ...
    function test_RBAC_UnauthorizedUserDenied() public {
        ...
        assertFalse(accessControl.checkPermission(unauthorizedUser, ResourceType.LABS, OperationType.CREATE));
    ...
    function test_RBAC_UserSpecificPermissions() public {
        ...
        assertTrue(accessControl.checkPermission(unauthorizedUser, ResourceType.LABS, OperationType.CREATE));
    }
    ...
    function test_Upgradeability_UnauthorizedUpgradeFails() public {
        ...
        vm.expectRevert("IntegratedLABSRegistry: RBAC permission denied");
    ...
    function test_GasOptimization_CreateLABS() public {
        // Measure gas for LABS creation
        uint256 gasStart = gasleft();
        
        vm.prank(user);
        bool success = integratedRegistry.createLABSIntegrated(
            TEST_LABS,
            TEST_PUBLIC_KEY,
    ...
    function test_GasOptimization_BatchCreateLABSs() public {
        ...
        vm.prank(user);
        bytes32 batchHash = integratedRegistry.batchCreateLABSsIntegrated(
            LABSs,
            publicKeys,
            serviceEndpoints
        );
        ...
        // Batch should be more efficient per LABS
        uint256 gasPerLABS = gasUsed / 3;
        assertTrue(gasPerLABS < 80000); // Should be less than individual creation
    }
    ...
    function test_GasOptimization_PerformanceMetrics() public {
        ...
        vm.prank(user);
        integratedRegistry.createLABSIntegrated(TEST_LABS, TEST_PUBLIC_KEY, TEST_SERVICE_ENDPOINT);
    ...
    function test_GasOptimization_PerformanceBenchmarks() public {
        ...
        // Check benchmark
        uint256 benchmark = integratedRegistry.getPerformanceBenchmark("CREATE_LABS");
    ...
    function test_Integration_AllFeaturesWorkTogether() public {
        // Create LABS with RBAC check and gas optimization
        vm.prank(user);
        bool LABSCreated = integratedRegistry.createLABSIntegrated(
            TEST_LABS,
            TEST_PUBLIC_KEY,
            TEST_SERVICE_ENDPOINT
        );
        assertTrue(LABSCreated);
    ...
    function test_Integration_FeatureToggles() public {
        ...
        // Should be able to create LABS without RBAC check
        vm.prank(unauthorizedUser);
        bool success = integratedRegistry.createLABSIntegrated(
            string(abi.encodePacked(TEST_LABS, "_no_rbac")),
            TEST_PUBLIC_KEY,
            TEST_SERVICE_ENDPOINT
        );
        assertTrue(success);
        ...
        vm.prank(unauthorizedUser);
        vm.expectRevert("IntegratedLABSRegistry: RBAC permission denied");
        integratedRegistry.createLABSIntegrated(
            string(abi.encodePacked(TEST_LABS, "_rbac_restored")),
            TEST_PUBLIC_KEY,
            TEST_SERVICE_ENDPOINT
        );
    }
    ...
    function test_Security_ZeroAddressProtection() public {
        ...
        new IntegratedLABSRegistry();
    }

    // ===== EDGE CASE TESTS =====
    
    function test_EdgeCase_EmptyStrings() public {
        // Should reject empty LABS
        vm.prank(user);
        vm.expectRevert("LABS cannot be empty");
        integratedRegistry.createLABSIntegrated("", TEST_PUBLIC_KEY, TEST_SERVICE_ENDPOINT);
        
        // Should reject empty public key
        vm.prank(user);
        vm.expectRevert("Public key cannot be empty");
        integratedRegistry.createLABSIntegrated(TEST_LABS, "", TEST_SERVICE_ENDPOINT);
    }
    ...
    function test_EdgeCase_ArrayLengthMismatch() public {
        ...
        integratedRegistry.batchCreateLABSsIntegrated(LABSs, publicKeys, serviceEndpoints);
    }
    
    function test_EdgeCase_DuplicateLABS() public {
        // Create LABS first
        vm.prank(user);
        integratedRegistry.createLABSIntegrated(TEST_LABS, TEST_PUBLIC_KEY, TEST_SERVICE_ENDPOINT);
        
        // Should fail to create same LABS again
        vm.prank(user);
