...
describe("LABS Governance System", function () {
    let governanceToken;
    let timelock;
    let governor;
    let governanceProxy;
    ...
    beforeEach(async function () {
        ...
        governanceToken = await LABSGovernanceToken.deploy();
        await governanceToken.deployed();
        
        // Deploy Timelock
        const LABSTimelock = await ethers.getContractFactory("LABSTimelock");
        timelock = await LABSTimelock.deploy(
            MIN_DELAY,
            [proposer.address],
            [executor.address],
            deployer.address
        ...
        governor = await LABSGovernor.deploy(governanceToken.address, timelock.address);
        ...
        // Deploy LABS Registry Implementation
        const LABSRegistry = await ethers.getContractFactory("EthereumLABSRegistry");
        LABSRegistry = await LABSRegistry.deploy();
        await LABSRegistry.deployed();
        
        // Deploy LABS Proxy
        const LABSProxy = await ethers.getContractFactory("LABSProxy");
        LABSProxy = await LABSProxy.deploy();
        await LABSProxy.deployed();
        
        // Initialize LABS Proxy
        await LABSProxy.initialize(LABSRegistry.address);
        ...
        // Enable governance control for LABS proxy
        await proxyAdmin.setGovernanceControl(LABSProxy.address, true);
        
        // Transfer LABS proxy ownership to proxy admin
        await LABSProxy.transferOwnership(proxyAdmin.address);
        ...
        it("Should allow proposal creation with sufficient voting power", async function () {
            ...
            const calldatas = [
                ethers.utils.defaultAbiCoder.encode(
                    ["address", "address"],
                    [LABSProxy.address, LABSRegistry.address]
                )
            ];
            const description = "Upgrade LABS Registry";
    ...
    describe("Contract Upgrade Process", function () {
        it("Should successfully execute a contract upgrade through governance", async function () {
            ...
            const LABSRegistryV2 = await ethers.getContractFactory("EthereumLABSRegistry");
            const newImplementation = await LABSRegistryV2.deploy();
            ...
            await governor.queue(
                ...
                [ethers.utils.defaultAbiCoder.encode(
                    ["address", "address"],
                    [LABSProxy.address, newImplementation.address]
                )],
                ethers.utils.keccak256(ethers.utils.toUtf8Bytes("Upgrade LABS Registry to V2"))
            );
            ...
            // Verify upgrade
            const currentImplementation = await governanceProxy.getProxyImplementation(LABSProxy.address);
        ...
        it("Should prevent direct upgrades when governance control is enabled", async function () {
            ...
            await expect(
                proxyAdmin.upgrade(LABSProxy, newImplementation.address)
